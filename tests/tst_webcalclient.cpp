...
void tst_WebCalClient::firstDownload()
{
    ...
    QVERIFY(mClient->init());
    QVERIFY(mClient->processData(icsDataFirst, "\"etag\"", message, &added, &deleted));
    QCOMPARE(added, static_cast<unsigned int>(1));
    QCOMPARE(deleted, static_cast<unsigned int>(0));
...
void tst_WebCalClient::downloadWithSameEtag()
{
    ...
    QVERIFY(mClient->init());
    QVERIFY(mClient->processData(icsDataFirst, "\"etag\"", message, &added, &deleted));
    QCOMPARE(added, static_cast<unsigned int>(0));
    QCOMPARE(deleted, static_cast<unsigned int>(0));
    QVERIFY(message.isEmpty());
...
void tst_WebCalClient::downloadWithDifferentEtag()
{
    ...
    QVERIFY(mClient->init());
    QVERIFY(mClient->processData(icsDataSecond, "\"etag2\"", message, &added, &deleted));
    QCOMPARE(added, static_cast<unsigned int>(2));
    QCOMPARE(deleted, static_cast<unsigned int>(1));
    QVERIFY(message.isEmpty());
...
void tst_WebCalClient::validateThird()
{
    ...
    QCOMPARE(ev->summary(), QStringLiteral("Rentrée scolaire des élèves - Zone B"));
}
...
void tst_WebCalClient::downloadWithoutEtag()
{
    ...
    QVERIFY(mClient->init());
    QVERIFY(mClient->processData(icsDataThird, "", message, &added, &deleted));
    QCOMPARE(added, static_cast<unsigned int>(1));
    QCOMPARE(deleted, static_cast<unsigned int>(2));
    QVERIFY(message.isEmpty());
